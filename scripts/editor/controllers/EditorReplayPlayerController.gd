...
func set_next_frame(frame:Replay.Frame):
	if is_instance_of(next_frame, Replay.SyncFrame):
		editor.sync_manager.seek(next_frame.sync_time)
...
func process_hitobject(object:EditorHitObject):
	...
	match frame.hit_state:
		EditorHitObject.HitState.HIT: object.hit()
		EditorHitObject.HitState.MISS: object.miss()
		EditorHitObject.HitState.NONE, _: pass
	queued_hit_frames.erase(frame)

func _process(_delta):
	process_frame_queue()
	if next_movement_frame != null:
		if editor.settings.camera.lock: _process_lock()
		else: _process_spin()
func _process_lock():
