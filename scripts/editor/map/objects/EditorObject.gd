...
@export var editor:EditorScene
@export var manager:EditorObjectManager
...
func _process(_delta):
	if editor == null or manager == null: return
	visible = get_visibility(editor.sync_manager.current_time)
	if !visible: return
	update(editor.sync_manager.current_time)
	update_transform()
