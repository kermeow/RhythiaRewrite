...
var controller:EditorPlayerController:
	set(value):
...
func _ready():
	...
	camera.fov = editor.settings.controls.fov
	absolute_camera.fov = editor.settings.controls.fov
	if local_player: # and !get_tree().vr_enabled:
		camera.make_current()
		if editor.settings.controls.absolute:
	...
	_real_cursor.scale = Vector3.ONE * editor.settings.skin.cursor.scale / 2.0
...
func _preprocess_cursor():
	...
	parallax *= editor.settings.camera.parallax.camera
...
func _postprocess_cursor():
	...
	if editor.settings.camera.drift:
...
func _relative_movement(offset:Vector2):
	var mouse_movement = offset * editor.settings.controls.sensitivity.mouse / 100.0
	if editor.settings.camera.lock:
...
func _process(_delta):
	...
	if editor.settings.skin.cursor.trail_enabled:
...
func _cursor_trail(_delta):
	...
	if gap_length > 0:
		var new_trails = floor(editor.settings.skin.cursor.trail_detail*gap_length)
		if new_trails > 0:
...
#					var position = end_position - gap * progress
	...
	for trail_no in total_trails:
		...
		var time_alpha = 1 - (time / editor.settings.skin.cursor.trail_length)
		var distance_alpha = 1 - (float(trail_no) / (editor.settings.skin.cursor.trail_distance * editor.settings.skin.cursor.trail_detail))
...
func _physics_process(_delta):
	...
	for object in objects:
		if editor.sync_manager.physics_time < object.spawn_time: break
...
func fail():
	...
	did_fail = true
	score.failed = true
	score.failed_at = editor.sync_manager.current_time
	if !editor.mods.no_fail:
		lock_score = true
		failed.emit()
