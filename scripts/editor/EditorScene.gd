...
@export_category("Editor Managers")
@export var sync_manager:EditorSyncManager
@export var object_manager:EditorObjectManager
@export var replay_manager:EditorReplayManager

@export_category("Other Nodes")
@export var origin:Node3D
@export var world_parent:EditorWorldContainerObject
@export var player:EditorPlayerObject
...
func _ready():
	if replay != null:
		...
		replay_manager.mode = EditorReplayManager.Mode.PLAY
		replay_manager.replay = replay
		var replay_controller = EditorPlayerController.ReplayController.new()
		player.controller = replay_controller
		call_deferred("add_child", replay_controller)
	else:
		var solo_controller = EditorPlayerController.SoloController.new()
	...
	if sync_manager is EditorAudioSyncManager: sync_manager.audio_stream = mapset.audio
	...
	EditorHitObject.hit_sound = $Hit
	EditorHitObject.miss_sound = $Miss
