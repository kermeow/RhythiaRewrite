extends EditorObjectRenderer
class_name EditorNoteRenderer
...
func prepare():
	...
	mesh = Rhythia.blocks.get_by_id(editor.settings.skin.block.mesh)
...
func render_objects(objects:Array):
	...
	for object in objects:
		...
		if not object is EditorNoteObject: continue
	...
	for i in count:
		...
		multimesh.set_instance_transform(i,note.global_transform
			...
			.scaled_local(Vector3.ONE * editor.settings.skin.block.scale))
