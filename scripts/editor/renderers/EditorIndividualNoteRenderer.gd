...
class_name EditorIndividualNoteRenderer

@onready var note:EditorNoteObject = get_parent()
@onready var editor:EditorScene = note.editor
...
func _ready():
	mesh_plus = Rhythia.blocks.get_by_id(editor.settings.skin.block.mesh)
	mesh = mesh_plus.mesh
	for i in mesh.get_surface_count():
		set_surface_override_material(i, mesh.surface_get_material(i).duplicate())
	color = note.colour
	transparency = 1
	scale_object_local(Vector3.ONE * editor.settings.skin.block.scale)
