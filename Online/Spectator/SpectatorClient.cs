...
namespace Rhythia.Online.Spectator
{
    public class SpectatorClient : HubClient, ISpectatorClient, ISpectatorServer
    {
        ...
        public async Task PlayerAdded(string userId, string userName)
        {
            ...
            Rhythia.Globals.Online.Instance.SpectatePlayerNames[userId] = userName;
            Rhythia.Globals.Online.Instance.SpectatePlayerMaps[userId] = "N/A";
        }

        public async Task PlayerRemoved(string userId)
        {
            GD.Print($"Player removed! {userId}");
            Rhythia.Globals.Online.Instance.SpectatePlayerNames.Remove(userId);
            Rhythia.Globals.Online.Instance.SpectatePlayerMaps.Remove(userId);
        }
        ...
        {
            Rhythia.Globals.Online.Instance.SpectatePlayerNames[userId] = streamInfo.UserName ?? "unknown";
            Rhythia.Globals.Online.Instance.SpectatePlayerMaps[userId] = streamInfo.MapId ?? "unknown";
            var exists = WatchingUsers.TryGetValue(userId, out var user);
            if (!exists || user is null) return;
